...
const ENDING_WARNINGS = {
  ...
  crisis: [
    ...
    },
    {
      title: '🔥 代價由誰承擔',
      lines: [
        '戰爭打起來，政客躲進防空洞，人民站在廢墟裡。',
        '那些喊著「打就打」的人，很少是第一線的那個人。',
  ...
  ccp_mode_win: [
    {
      ...
      lines: [
        '中共在遊戲裡拿下台灣——但這只是遊戲。',
        '現實中，每一個玩這個陣營的人都應該問自己：我真的支持這套體制嗎？',
        '中共的體制連自己的人民都不放過，你以為你會是例外？',
        '玩過中共陣營，更要理解為什麼台灣值得守護。'
      ]
    },
    {
      title: '🪤 你成了棋子',
      lines: [
        '在中共的邏輯裡，每個人都是可以被消耗的資源。',
        '馬雲消失了。任志強消失了。秦剛消失了。',
        '功勞越大，威脅越大，被收割的速度越快。',
        '中共沒有永遠的盟友，只有永遠的利益。'
      ]
    },
    {
      title: '⚙️ 機器不在乎零件',
      lines: [
        '恭喜你讓這台機器運作成功。但機器不感謝任何一個零件。',
        '習總、王外長、胡總編——他們終究只是更大機器裡的螺絲。',
        '每一個威權政體裡的「功臣」，都等著被下一任清算。',
        '這場遊戲的真正問題不是誰赢，而是這套體制是否應該存在。'
      ]
    },
    {
      title: '🔮 統一之後',
      lines: [
        '假設中共統一台灣。然後呢？',
        '台灣人的言論自由、選舉權、新聞自由——全部消失。',
        '你的孩子、你的家人，從此活在監控和審查裡。',
        '玩過這局，思考一下：這是你想要的結局嗎？'
      ]
    },
  ],
  ...
  ccp_mode_lose: [
    {
      ...
      lines: [
        '你用盡了中共所有的手段，台灣還是撐住了。',
        '這不是因為台灣有什麼特別的魔法。',
        '而是因為民主社會在危機中能夠自我修復和動員。',
        '這就是威權最難以理解、也最難以對抗的東西。'
      ]
    },
    {
      title: '📖 歷史的判決',
      lines: [
        '中共輸了這局。但現實中的台灣還在持續面對壓力。',
        '每一個了解台灣處境的人，都是台灣的一道防線。',
        '把這場遊戲的體驗告訴身邊的人：威脅是真實的，抵抗也是可能的。',
        '台灣的故事還沒結束。你選擇站在哪一邊？'
      ]
    },
  ],
  ...
  ccp_mode_crisis: [
    {
      ...
      lines: [
        '戰爭爆發——這是所有選項裡最糟的一個。',
        '無論是台灣人還是中國人，承擔後果的都是老百姓。',
        '每一個讓戰爭成真的決策，都有人在為它付出生命。',
        '阻止戰爭，從理解對方的真實需求開始——而不是征服。'
      ]
    },
  ],
};
...
// 隨機抽取警語（依玩家陣營 + 結局勝負）
function drawWarning(endingId, winner, playerFaction) {
  let pool;
  ...
  if (playerFaction === 'ccp') {
    ...
    if (winner === 'ccp') {
      // 中共贏 → 中共模式勝利警語
      pool = ENDING_WARNINGS.ccp_mode_win;
    } else if (winner === 'crisis') {
      // 戰爭爆發 → 中共模式危機警語
      pool = ENDING_WARNINGS.ccp_mode_crisis;
    } else {
      // 台灣守住（中共輸）→ 中共模式落敗警語
      pool = ENDING_WARNINGS.ccp_mode_lose;
    }
  } else {
    // 台灣模式
    if (winner === 'tw') {
      pool = ENDING_WARNINGS.tw_win;
    } else if (winner === 'crisis') {
      pool = ENDING_WARNINGS.crisis;
    } else {
      pool = ENDING_WARNINGS.ccp_win;
    }
  }
